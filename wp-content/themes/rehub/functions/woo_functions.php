...
if( class_exists( 'WeDevs_Dokan' ) OR defined( 'wcv_plugin_dir' ) ) {
...
if ( !function_exists('rh_show_vendor_info_single') ) {
	function rh_show_vendor_info_single() {
		...
		if( class_exists( 'WeDevs_Dokan' ) ) {
			...
			$verified_vendor = get_user_meta( $vendor_id, 'dokan_feature_seller', true );
		} else {
		...
		$sold_by = ( $is_vendor ) ? sprintf( '<h5><a href="%s" class="wcvendors_cart_sold_by_meta">%s</a></h5>', $store_url, $store_name ) : '<h5>'. get_bloginfo( 'name' ) .'</h5>';
		...
		if(class_exists( 'WeDevs_Dokan' ) && dokan_get_option( 'contact_seller', 'dokan_general', 'on' ) == 'on'){
			...
			echo ' <a href="'.$store_url.'#dokan-contact" class="vendor_store_owner_contactlink'.$class.'"><i class="fa fa-envelope-o" aria-hidden="true"></i> <span>'. __('Ask owner', 'rehub_framework') .'</span></a>';									
			echo '</span>';					
		}	
	...
	*/
	...
	function custom_dokan_toc_url( $tabs ){
		$tabs['terms_and_conditions'] = array(
			'title' => apply_filters( 'dokan_about_store_title', __( 'Terms and Conditions', 'dokan' ) ),
			'url'   => '#vendor-about'
		);
		return $tabs;
	}
	...
	function dokan_loop_sold_by() {
		...
		$sold_by = dokan_is_user_seller( $vendor_id )
			...
			: get_bloginfo( 'name' );
		?>
		<small class="wcvendors_sold_by_in_loop"><?php echo apply_filters( 'dokan_sold_by_label', __( 'Sold by', 'rehub_framework' ) ); ?> <?php echo $sold_by; ?></small><br />
		<?php
	}
