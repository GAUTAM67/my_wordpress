	...
	<?php global $woocommerce; if($woocommerce) :?>
		...
		<?php $products = new WP_Query( $args ); if ( $products->have_posts() ) : ?>
    		<?php while ( $products->have_posts() ) : $products->the_post(); global $product?>
	            ...
	            <?php 
	            	$rehub_woodeals_short = get_post_meta($product->get_id(), 'rehub_woodeals_short', true );
	            	$rehub_woodeals_short_side = get_post_meta($product->get_id(), 'rh_code_incart', true );
	            	$woo_aff_links_inreview = ($rehub_woodeals_short !='' || $rehub_woodeals_short_side !='') ? '1' : ''; ?>
	            <?php $offer_coupon = get_post_meta( $product->get_id(), 'rehub_woo_coupon_code', true ) ?>
	            <?php $offer_coupon_date = get_post_meta( $product->get_id(), 'rehub_woo_coupon_date', true ) ?>
	            <?php $offer_coupon_mask = get_post_meta( $product->get_id(), 'rehub_woo_coupon_mask', true ) ?>
...
function set_content_end($content) {
	...
	if(rehub_option('enable_button_bottom') == 1){
