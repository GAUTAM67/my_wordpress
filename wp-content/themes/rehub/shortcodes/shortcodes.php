		...
		?>
			...
			<div class="egg_sort_list re_sort_list simple_sort_list mb20">
				...
				<?php while ( $wp_query->have_posts() ) : $wp_query->the_post();  global $product;  ?>
					<?php $woolink = ($product->get_type() =='external' && $product->add_to_cart_url() !='') ? $product->add_to_cart_url() : get_post_permalink($product->get_id()) ;?>  
		            <div class="rehub_feat_block table_view_block">               
		                <div class="logo_offer_thumb offer_thumb<?php if(!empty($logo)) {echo ' nologo_thumb';}?>">  
						<?php if ($show_vendor==1) :?>
						...
						<?php else:?>
							<?php 
								$brand_url = get_post_meta($product->get_id(), 'rehub_woo_coupon_logo_url', true );
        ...
        <div class="rh-tabletext-block">
	        ...
	        <div class="rh-tabletext-block-wrapper flowhidden"> 
	            ...
	            <div class="single_compare_right">
                    ...
                    <?php endif;?>
	                <?php echo do_shortcode('[content-egg-block template=custom/all_merchant_widget post_id="'.$post_id.'"]');?>
	            </div> 
			</div>
		</div>
		...
		<div class="commbox">
			<div class="comment-author vcard clearfix">                    
				...
				<div class="comm_meta_wrap">
					...
					<?php else:?>
						<span class="fn"><?php echo get_comment_author_link($comment->comment_ID); ?>
						<?php 	
							if (function_exists('bp_get_member_type')){	
								$author_id = $comment->user_id;		
					...
					<?php endif;?>				
					<span class="time"><?php echo get_comment_date(get_option( 'date_format' ), $comment->comment_ID) ?></span>
                </div>				
			</div>
			<?php if (rehub_option('type_user_review') == 'full_review' || rehub_option('type_user_review') == 'user') :?>               
	          	<?php $userCriteria = get_comment_meta($comment->comment_ID, 'user_criteria', true);
